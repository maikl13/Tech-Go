        ...
        <div class="col-md-12">
            <div class="tile">
                <div class="tile-body">
                    ...
                    <form method="POST" action="/admin/employees/{{$item->id}}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <textarea class="body form-control" name="description_ar">{!!$item->description_ar!!}</textarea>

                        <label>وصف انجليزي</label>
                        <textarea class="body form-control" name="description_en">{!!$item->description_en!!}</textarea> -->

                        <label>صورة</label>
                        <input type="file" name="image" class="form-control"
                               accept="image/*" >
                        <img src="/uploads/teams/{{$item->image}}" style="width:150px;display:block" />
                        ...
                        <select class="form-control" required name="position_id">
                            ...
                            <option @if($p->id == $item->position_id) selected @endif value="{{$p->id}}">{{$p->title_ar}} / {{$p->title_en}}</option>
                            @endforeach
                        </select>
